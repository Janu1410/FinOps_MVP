      ...
      >
        ...
        <motion.div variants={slideUpVariants} className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-4 leading-tight">
            Finalize Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8B2FC9] to-blue-500">Slot</span>
          </h1>
          ...
          {inquiry && (
            <div className="bg-white/[0.02] border-b border-white/5 p-6">
              <div className="flex items-center gap-2 text-sm font-bold text-gray-500 uppercase tracking-wide mb-4">
                <CheckCircle2 size={14} className="text-[#8B2FC9]" /> Inquiry Details
              </div>
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <motion.div variants={formItemVariants} className="space-y-2">
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide block">Select Date</label>
                <div className="relative">
                    ...
                    />
                    <Calendar className="absolute right-3.5 top-3.5 text-gray-500 pointer-events-none" size={16} />
                </div>
              </motion.div>

              {/* Timezone */}
              <motion.div variants={formItemVariants} className="space-y-2">
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide block">Your Timezone</label>
                <div className="relative">
                    <input
                        disabled
                        value={timezone}
                    ...
                    />
                    <MapPin className="absolute left-3.5 top-3.5 text-gray-600" size={16} />
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={formItemVariants} className="space-y-4">
              <div className="flex justify-between items-end">
                  <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Available Slots</label>
                  {selectedDate && (
              ...
              <div className="min-h-[120px]">
                  ...
                  {!loadingSlots && slots.length === 0 && selectedDate && (
                    <div className="flex flex-col items-center justify-center h-32 border border-dashed border-white/10 rounded-xl bg-white/[0.01] text-center">
                      ...
                      <p className="text-gray-600 text-xs">Try selecting another date.</p>
                    </div>
                  )}
                  ...
                  {!loadingSlots && !selectedDate && (
                    <div className="flex flex-col items-center justify-center h-32 border border-dashed border-white/10 rounded-xl bg-white/[0.01] text-center">
                      ...
                      <p className="text-gray-400 text-sm">Please select a date above</p>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {slots.map((slot, idx) => (
                        ...
                        >
                          ...
                          <span className={`text-[10px] ${selectedSlot?.start === slot.start ? "text-white/80" : "text-gray-600 group-hover:text-gray-500"}`}>
                             - {slot.end}
                          </span>
            ...
            {/* 3. Action Button */}
            <motion.div variants={formItemVariants} className="pt-4 border-t border-white/10">
              <button
